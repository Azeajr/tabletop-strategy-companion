...
class SqliteClient {
  ...
  async transaction(fn: () => Promise<void>): Promise<void> {
    // Reentrant: nested transactions just run inline.
    if (this.txDepth > 0) {
      this.txDepth++
...
// Stub for type-checking. Vitest alias replaces this whole module with
// sqlite-test-client.ts which provides a real implementation.
